...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='val',
        help='Options for dataset, should be val or test.')
    parser.add_argument('--out', help='Output result file in pickle format.')
    parser.add_argument('--output-txt', help='Output result file name in .txt format.')
...
#                 filenames.append(img_meta['ori_filename'].rsplit('/', maxsplit=1)[1]) #
                filenames.append(img_meta['filename'].rsplit('/', maxsplit=1)[1]) #
        except:
            # harddisk data
            img_metas = data['img_metas'][0].data[0]
            for i, img_meta in enumerate(img_metas):
...
def main():
    ...
    output_filename = args.output_txt
    print(f"Trying to run dataset: {dataset_type}")
    ...
    samples_per_gpu = 1
    cfg_data = getattr(cfg.data, dataset_type)
    if isinstance(cfg_data, dict):
        ...
        if samples_per_gpu > 1:
            ...
            if cfg_data.get('pipeline', None) is not None:
                # Replace 'ImageToTensor' to 'DefaultFormatBundle'
                cfg_data.pipeline = replace_ImageToTensor(cfg_data.pipeline)
    elif isinstance(cfg_data, list):
        for ds_cfg in cfg_data:
            ds_cfg.test_mode = True
        samples_per_gpu = max(
            [ds_cfg.pop('samples_per_gpu', 1) for ds_cfg in cfg_data])
        if samples_per_gpu > 1:
            for ds_cfg in cfg_data:
...
#     dataset = build_dataset(cfg.data.test, dict(test_mode=True))
    ...
    if rank == 0:
        ...
        if args.format_only:
            dataset.format_results(outputs, filenames, output_filename, **kwargs)
        if args.eval:
